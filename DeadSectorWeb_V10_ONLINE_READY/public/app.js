...
function playerMesh(){const m=new THREE.Mesh(new THREE.CapsuleGeometry(.3,.9,4,8),new THREE.MeshStandardMaterial({color:0x7397a5}));m.position.y=1;return m}
...
addEventListener("keydown",e=>{keys[e.key.toLowerCase()]=true;audio();if(e.key.toLowerCase()==="r")socket.emit("reload");if(e.key.toLowerCase()==="e")interact()});
...
function syncVisuals(now){
 ...
 for(const p of S.players){if(p.id===myId)continue;let g=players.get(p.id);if(!g){g=playerMesh();scene.add(g);players.set(p.id,g)}g.position.lerp(new THREE.Vector3(p.x,0,p.z),.35);g.visible=!p.down}
...
document.querySelector("#br > div:first-child").textContent=({AR15:"AR-15",SMG:"Viper SMG",SHOTGUN:"Ranger Shotgun",DMR:"Sentinel DMR",LMG:"Atlas LMG",VOLT:"Volt-9 Energy Pistol"})[p.weapon]||p.weapon;document.getElementById("hp").style.width=(p.hp/p.maxHp*100)+"%";
