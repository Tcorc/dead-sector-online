...
const BASE_WALLS=[
  {x:-44,z:0,w:1,d:76},{x:44,z:0,w:1,d:76},{x:0,z:-38,w:88,d:1},{x:0,z:38,w:88,d:1},

  {x:-18,z:-26,w:1,d:24},{x:18,z:-26,w:1,d:24},
  {x:-31,z:-14,w:26,d:1},{x:-11,z:-14,w:8,d:1},{x:0,z:-14,w:14,d:1},{x:11,z:-14,w:8,d:1},{x:31,z:-14,w:26,d:1},

  {x:-31,z:0,w:1,d:28},{x:-24,z:14,w:14,d:1},{x:-38,z:14,w:12,d:1},
  {x:31,z:0,w:1,d:28},{x:24,z:14,w:14,d:1},{x:38,z:14,w:12,d:1},
  ...
  {x:-32,z:34,w:24,d:1},{x:32,z:34,w:24,d:1},{x:-20,z:32,w:1,d:4},{x:20,z:32,w:1,d:4},

  {x:-9,z:2,w:8,d:1},{x:9,z:2,w:8,d:1},{x:-11,z:10,w:8,d:1},{x:11,z:10,w:8,d:1}
];

const DOORS={
  leftHall:{x:-18,z:-14,w:6,d:1},
  rightHall:{x:18,z:-14,w:6,d:1},
  stage:{x:0,z:22,w:8,d:1},
  backstage:{x:0,z:30,w:8,d:1}
};
...
addEventListener("keydown",e=>{
 ...
 if(k==="e")interact();
 if(k==="q")socket.emit("swapWeapon");

 // Direct weapon slot selection.
 if(e.code==="Digit1"||e.code==="Numpad1"){
   e.preventDefault();
   socket.emit("selectWeaponSlot",{slot:0});
 }
 if(e.code==="Digit2"||e.code==="Numpad2"){
   e.preventDefault();
   socket.emit("selectWeaponSlot",{slot:1});
 }
});
...
socket.on("weaponSelected",d=>{
  tone(240,.05,"square",.02,50);
  powerFlash(`WEAPON SLOT ${d.slot+1}`);
});
...
function nearby(){
 ...
 const opts=[
  ...
  {x:0,z:22,type:"door:stage",label:"OPEN STAGE — 1250",active:!S.doors.stage},
  {x:0,z:30,type:"door:backstage",label:"OPEN BACKSTAGE — 1500",active:!S.doors.backstage},
  ...
  {x:12,z:31,type:"doubletap",label:"BUY DOUBLE TAP — 2000",active:S.doors.backstage&&!p.perkDouble},
  {x:-12,z:31,type:"staminup",label:"BUY STAMIN-UP — 2000",active:S.doors.backstage&&!p.perkStaminup},

  {x:-6,z:-20,type:"ammo",label:"BUY AMMO — 500",active:true},
  {x:-14,z:15,type:"mysterybox",label:"MYSTERY BOX — 950",active:true},
  {x:0,z:27,type:"pack",label:lvl>=3?"MAX WEAPON UPGRADE":`UPGRADE WEAPON LVL ${lvl+1} — ${[0,5000,15000,30000][lvl+1]}`,active:S.doors.stage},
  ...
  {x:-13,z:15,type:"wall_shotgun",label:"BUY RANGER SHOTGUN — 1500",active:true},
  {x:13,z:15,type:"wall_dmr",label:"BUY SENTINEL DMR — 1800",active:true}
 ];
...
camera.position.y=1.65+elevation;camera.rotation.order="YXZ";camera.rotation.y=yaw;camera.rotation.x=pitch;if(p){document.getElementById("round").textContent="ROUND "+S.round;document.getElementById("roomLabel").textContent="ROOM "+roomCode;document.getElementById("threats").textContent=S.phase==="intermission"?"BREAK":("ZOMBIES "+(S.zombies.length+S.queue));const shownPoints=Math.floor(Number(p.points)||0);document.getElementById("points").textContent=shownPoints+" PTS";const moneyHUD=document.getElementById("moneyTop");if(moneyHUD)moneyHUD.textContent="$"+shownPoints;document.getElementById("ammo").textContent=p.ammo+" / "+p.reserve;
