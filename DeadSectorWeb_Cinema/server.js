...
const BASE_WALLS=[
  // OUTER SHELL
  {x:-44,z:0,w:1,d:76},{x:44,z:0,w:1,d:76},
  {x:0,z:-38,w:88,d:1},{x:0,z:38,w:88,d:1},

  // FOYER. The north wall is split ONLY at the two purchasable hall doors.
  {x:-18,z:-26,w:1,d:24},{x:18,z:-26,w:1,d:24},
  {x:-31,z:-14,w:26,d:1},
  {x:-11,z:-14,w:8,d:1},
  {x:0,z:-14,w:14,d:1},
  {x:11,z:-14,w:8,d:1},
  ...
  // LEFT HALL ENCLOSURE
  {x:-31,z:0,w:1,d:28},
  {x:-24,z:14,w:14,d:1},{x:-38,z:14,w:12,d:1},

  // RIGHT HALL ENCLOSURE
  {x:31,z:0,w:1,d:28},
  {x:24,z:14,w:14,d:1},{x:38,z:14,w:12,d:1},

  // MAIN AUDITORIUM SIDES
  {x:-18,z:4,w:1,d:36},{x:18,z:4,w:1,d:36},

  // AUDITORIUM -> STAGE wall. Only the 8-wide stage doorway is openable.
  {x:-11,z:22,w:14,d:1},{x:11,z:22,w:14,d:1},

  // STAGE -> BACKSTAGE wall. Only the 8-wide backstage doorway is openable.
  {x:-22,z:30,w:20,d:1},{x:-6,z:30,w:8,d:1},{x:10,z:30,w:24,d:1},

  // BACKSTAGE SHELL
  {x:-32,z:34,w:24,d:1},{x:32,z:34,w:24,d:1},
  {x:-20,z:32,w:1,d:4},{x:20,z:32,w:1,d:4},

  // AUDITORIUM COVER
  {x:-9,z:2,w:8,d:1},{x:9,z:2,w:8,d:1},
  {x:-11,z:10,w:8,d:1},{x:11,z:10,w:8,d:1}
];

const DOORS={
  leftHall:{x:-18,z:-14,w:6,d:1,cost:1000,zone:4},
  rightHall:{x:18,z:-14,w:6,d:1,cost:1000,zone:1},
  stage:{x:0,z:22,w:8,d:1,cost:1250,zone:3},
  backstage:{x:0,z:30,w:8,d:1,cost:1500,zone:3}
};
...
const ZOMBIE_WINDOWS=[
  ...
  {x:22,z:-37.4,side:"south",zone:0},
  {x:-43.4,z:8,side:"west",zone:4},
  {x:43.4,z:8,side:"east",zone:1},
  {x:-18,z:37.4,side:"north",zone:3},
  {x:18,z:37.4,side:"north",zone:3}
];

const SPAWNS={
  0:[[-22,-36.6],[0,-36.6],[22,-36.6],[-12,-22],[12,-22]],
  1:[[40,4],[38,10],[35,-2],[27,8]],
  2:[[-14,5],[0,8],[14,5],[-12,16],[12,16]],
  3:[[-18,36.5],[18,36.5],[-10,33],[10,33]],
  4:[[-40,4],[-38,10],[-35,-2],[-27,8]],
  5:[[-36,-28],[-33,-32]],
  6:[[36,-28],[33,-32]]
};
...
io.on("connection",socket=>{
  ...
  socket.on("selectWeaponSlot",({slot})=>{
    const r=rooms.get(socket.data.room),p=r?.players.get(socket.id);
  ...
  socket.on("buy",({type})=>{
    ...
    const sendState=()=>io.to(r.code).emit("state",publicState(r));
    const deny=(cost,label)=>socket.emit("purchaseDenied",{
      type,label,cost,points:Math.floor(p.points),needed:Math.max(0,Math.ceil(cost-p.points))
    });
    const ok=(label)=>{
    ...
    const doors={
      "door:leftHall":{key:"leftHall",cost:1000,zones:[4,5],label:"LEFT HALL OPEN"},
      "door:rightHall":{key:"rightHall",cost:1000,zones:[1,6],label:"RIGHT HALL OPEN"},
      "door:stage":{key:"stage",cost:1250,zones:[3],label:"STAGE OPEN"},
      "door:backstage":{key:"backstage",cost:1500,zones:[3],label:"BACKSTAGE OPEN"}
    };

    if(doors[type]){
      const d=doors[type];
      if(r.doors[d.key])return ok(d.label);
      if(p.points<d.cost)return deny(d.cost,d.label);
      p.points-=d.cost;
      r.doors[d.key]=true;
      for(const zone of d.zones)r.activeZones.add(zone);
      r.doorRevision=(r.doorRevision||0)+1;
      for(const z of r.zombies.values()){z._path=[];z._nextPathAt=0;}
      return ok(d.label);
    }

    if(type==="ammo"){
      const cost=500;
      if(p.points<cost)return deny(cost,"AMMO");
      p.points-=cost;
      const w=activeWeapon(p);
      p.reserve=Math.min(w.reserve*2,p.reserve+w.reserve);
      saveActiveAmmo(p);
      return ok("AMMO PURCHASED");
    }

    if(type==="mysterybox"){
      const cost=950;
      if(p.points<cost)return deny(cost,"MYSTERY BOX");
      p.points-=cost;
    ...
    if(type==="pack"){
      ...
      if(level>=3){
        ...
        return ok("WEAPON MAXED");
      }
      const next=level+1,cost=PACK_COSTS[next];
      if(p.points<cost)return deny(cost,`UPGRADE LEVEL ${next}`);
      p.points-=cost;
      ...
      return ok(`${w.name} UPGRADED LEVEL ${next}`);
    }

    if(PERKS[type]){
      const names={jug:"JUGGERNOG",speed:"SPEED COLA",doubletap:"DOUBLE TAP",staminup:"STAMIN-UP"};
      const owned=(type==="jug"&&p.perkJug)||(type==="speed"&&p.perkSpeed)||(type==="doubletap"&&p.perkDouble)||(type==="staminup"&&p.perkStaminup);
      if(owned)return ok(names[type]+" OWNED");
      const cost=PERKS[type].cost;
      if(p.points<cost)return deny(cost,names[type]);
      p.points-=cost;
      applyPerk(p,type);
      io.to(r.code).emit("perkBought",{playerId:p.id,type});
      return ok(names[type]+" PURCHASED");
    }
  });
