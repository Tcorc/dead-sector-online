...
const floor=new THREE.Mesh(new THREE.PlaneGeometry(42,42),new THREE.MeshStandardMaterial({color:0x252722,roughness:1}));floor.rotation.x=-Math.PI/2;scene.add(floor);scene.add(new THREE.GridHelper(42,21,0x30332f,0x20221f));

const BASE_WALLS=[{x:-21,z:0,w:1,d:42},{x:21,z:0,w:1,d:42},{x:0,z:-21,w:42,d:1},{x:0,z:21,w:42,d:1},{x:0,z:-11,w:1,d:18},{x:0,z:11,w:1,d:18},{x:4,z:8,w:8,d:1},{x:16,z:8,w:8,d:1}];
const EAST_DOOR={x:0,z:0,w:1,d:4},LAB_DOOR={x:10,z:8,w:4,d:1};
...
BASE_WALLS.forEach(w=>box(w));const eastDoor=box(EAST_DOOR,3,0x6b5635),labDoor=box(LAB_DOOR,3,0x6b5635);
...
station(-12,-4,0x7b2727,"JUGGERNOG");
station(7,-10,0x2e6f91,"SPEED COLA");
station(15,12,0x8b3a26,"DOUBLE TAP");
station(6,13,0xc28f2c,"STAMIN-UP");
station(-5,6,0x806f2f,"AMMO");
station(-15,12,0x76551f,"MYSTERY BOX");
station(-17,-8,0x536b7d,"AR-15 WALL BUY");
station(-17,7,0x457d66,"VIPER SMG WALL BUY");
station(8,-16,0x7d6045,"SHOTGUN WALL BUY");
station(17,14,0x5b5d82,"DMR WALL BUY");
station(12,13,0x7c2d88,"WEAPON UPGRADE");
...
let pos=new THREE.Vector3(-8,1.65,0),yaw=0,pitch=0,keys={},shooting=false,aiming=false,lastShot=0,last=performance.now(),lastPadReload=false,lastPadInteract=false,lastGroan=0,stamina=100,reloadFinishLocal=0,mobileMoveX=0,mobileMoveY=0,mobileFire=false,mobileADS=false,mobileSprint=false,mobileLookDX=0,mobileLookDY=0;
function solids(){const a=[...BASE_WALLS];if(!S?.doors?.east)a.push(EAST_DOOR);if(!S?.doors?.lab)a.push(LAB_DOOR);return a}
...
function moveLocal(dx,dz){let nx=Math.max(-20.2,Math.min(20.2,pos.x+dx)),nz=Math.max(-20.2,Math.min(20.2,pos.z+dz));if(!blocked(nx,pos.z))pos.x=nx;if(!blocked(pos.x,nz))pos.z=nz}
...
addEventListener("keydown",e=>{keys[e.key.toLowerCase()]=true;audio();if(e.key.toLowerCase()==="r")socket.emit("reload");if(e.key.toLowerCase()==="e")interact();if(e.key.toLowerCase()==="q")socket.emit("swapWeapon")});
...
socket.on("packResult",d=>{pickupSound();powerFlash(`${d.name} — UPGRADE LEVEL ${d.level}`);});
...
function nearby(){const p=self();if(!p||!S)return null;const opts=[
{x:0,z:0,type:"eastDoor",label:"OPEN EAST WING — 1000",active:!S.doors.east},
{x:10,z:8,type:"labDoor",label:"OPEN LAB — 1500",active:!S.doors.lab},
{x:-12,z:-4,type:"jug",label:"BUY JUGGERNOG (200 HP) — 2500",active:!p.perkJug},
{x:7,z:-10,type:"speed",label:"BUY SPEED COLA (FAST RELOAD) — 3000",active:S.doors.east&&!p.perkSpeed},
{x:15,z:12,type:"doubletap",label:"BUY DOUBLE TAP (MORE DAMAGE) — 2000",active:S.doors.lab&&!p.perkDouble},
{x:6,z:13,type:"staminup",label:"BUY STAMIN-UP (FASTER/LONGER SPRINT) — 2000",active:S.doors.lab&&!p.perkStaminup},
{x:-5,z:6,type:"ammo",label:"BUY AMMO — 500",active:true},
{x:-17,z:-8,type:"wall_ar",label:"BUY AR-15 — 1200",active:true},
{x:-17,z:7,type:"wall_smg",label:"BUY VIPER SMG — 1000",active:true},
{x:8,z:-16,type:"wall_shotgun",label:"BUY RANGER SHOTGUN — 1500",active:S.doors.east},
{x:17,z:14,type:"wall_dmr",label:"BUY SENTINEL DMR — 1800",active:S.doors.lab},
{x:12,z:13,type:"pack",label:(p.packLevel||0)>=3?"MAX WEAPON UPGRADE":`UPGRADE WEAPON LVL ${(p.packLevel||0)+1} — ${[0,5000,15000,30000][(p.packLevel||0)+1]||0}`,active:S.doors.lab&&(p.packLevel||0)<3},

{x:-15,z:12,type:"mysterybox",label:"MYSTERY BOX — 950",active:true}
];let best=null,bd=2.25;for(const o of opts){if(!o.active)continue;const d=Math.hypot(pos.x-o.x,pos.z-o.z);if(d<bd){bd=d;best=o}}for(const q of S.players){if(q.id===myId||!q.down)continue;const d=Math.hypot(pos.x-q.x,pos.z-q.z);if(d<bd){bd=d;best={type:"revive",playerId:q.id,label:"REVIVE "+q.name}}}return best}
...
function syncVisuals(now){
 if(!S)return;eastDoor.visible=!S.doors.east;labDoor.visible=!S.doors.lab;
 ...
 for(const p of S.players){if(p.id===myId)continue;let g=players.get(p.id);if(!g){g=playerMesh();scene.add(g);players.set(p.id,g)}g.position.lerp(new THREE.Vector3(p.x,0,p.z),.35);
   g.visible=true;
...
const dx=(-Math.sin(yaw)*f+Math.cos(yaw)*r)*sp*dt,dz=(-Math.cos(yaw)*f-Math.sin(yaw)*r)*sp*dt;moveLocal(dx,dz);socket.emit("move",{x:pos.x,z:pos.z,yaw});if(shooting||gp?.fire||mobileFire)fire()}camera.position.copy(pos);camera.rotation.order="YXZ";camera.rotation.y=yaw;camera.rotation.x=pitch;if(p){document.getElementById("round").textContent="ROUND "+S.round;document.getElementById("roomLabel").textContent="ROOM "+roomCode;document.getElementById("threats").textContent=S.phase==="intermission"?"BREAK":("ZOMBIES "+(S.zombies.length+S.queue));document.getElementById("points").textContent=p.points+" PTS";document.getElementById("ammo").textContent=p.ammo+" / "+p.reserve;
