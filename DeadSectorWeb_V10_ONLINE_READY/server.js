...
const WEAPONS={
  AR15:{name:"AR-15",damage:40,fireRate:115,mag:30,reserve:180,headMult:2.5},
  SMG:{name:"Viper SMG",damage:27,fireRate:78,mag:40,reserve:240,headMult:2.25},
  SHOTGUN:{name:"Ranger Shotgun",damage:24,fireRate:620,mag:8,reserve:64,pellets:7,headMult:1.7},
  DMR:{name:"Sentinel DMR",damage:78,fireRate:280,mag:12,reserve:84,headMult:2.8},
  LMG:{name:"Atlas LMG",damage:36,fireRate:92,mag:75,reserve:300,headMult:2.2},
  VOLT:{name:"Volt-9 Energy Pistol",damage:150,fireRate:330,mag:12,reserve:84,headMult:2.0}
};
const BOX_POOL=["SMG","SHOTGUN","DMR","LMG","AR15","VOLT"];
...
function publicState(r){
  return {
    ...
    players:[...r.players.values()].map(p=>({...p})),
...
function activeWeapon(p){return WEAPONS[p.weapon]||WEAPONS.AR15;}
function refillWeapon(p){
  const w=activeWeapon(p);
  p.ammo=w.mag;p.reserve=w.reserve;
}
function pickupDrop(r,p,d,now){
  if(d.type==="maxammo"){
    for(const q of r.players.values())refillWeapon(q);
...
function resetPlayerForMatch(p){
  ...
  p.weapon="AR15"; p.ammo=base.mag; p.reserve=base.reserve;
...
io.on("connection",socket=>{
  socket.on("joinRoom",({room,name})=>{
    ...
    const p={
      ...
      weapon:"AR15",ammo:base.mag,reserve:base.reserve,
  ...
  socket.on("shoot",({targetId,headshot=false})=>{
    ...
    const now=Date.now(),w=activeWeapon(p);
  ...
  socket.on("buy",({type})=>{
    ...
    if(type==="eastDoor"&&!r.doors.east&&p.points>=1000){
      ...
      p.weapon=pick;
      refillWeapon(p);
      socket.emit("boxResult",{weapon:pick,name:WEAPONS[pick].name});
    }else if(PERKS[type]){
